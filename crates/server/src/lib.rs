...
use wasm_tutorial_shared::messages::*;
...
fn handle_todo_route(message: Message) -> Response {
    ...
    println!("crated data interface");
    match message.kind {
        MessageType::GetAll => {
            println!("get all message");
            let values = data.get_todos();
            let out_message = Message::get_all_server(&values);
            println!("{:?}", out_message);
            let body = out_message.to_bytes();
            Response::new().with_body(body)
        },
        MessageType::Add => {
            println!("add message");
            let mut todos = message.todos();
