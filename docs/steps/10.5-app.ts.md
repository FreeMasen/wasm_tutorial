...
<div class="explain">
<p>At the top of this file we add an event handler to the window's <code>DOMContentLoaded</code> event that will instantite our <code>App</code> class into a global variable. Then we have an array of mocked todo items. </p>
<p>The rest of this file is dedicated to our <code>App</code> class. This fill will act as an interface between the user and our data. Eventually we are going to insert out <code>bincode_parser</code> logic into this but right now we just need to make sure everything works.</p>
</div>
...
class App {
    ...
    removeToDo(id: number) {
        this.todos = this.todos.fileter(t => t.id == t.id)
    }
    /**
